...
vec_bus filtr_eks(const vec_bus& rout, std::function<bool(const bus_route&)> predicate) {
    ...
    for (const auto& route : rout) {
        ...
        if (predicate(route)) {filtr_vec.push_back(route);}
    }

    // Проверяем, нашли ли мы какие-либо маршруты
    if (filtr_vec.empty()) {
        std::cout << "Не найдено ни одного маршрута, соответствующего заданным критериям." << std::endl;
    }
    return filtr_vec; 
}
