...
vec_bus readd(const std::string &filename, size_t &count)
{
    ...
    fin.seekg(0, std::ios::beg); // Возвращаемся в начало файла
    ...
    {
        fin >> routes[i].nomer.double_nomer;
        fin.ignore(); // Игнорируем символ новой строки
        fin.getline(routes[i].typ_bus, sizeof(routes[i].typ_bus));
        fin.getline(routes[i].punkt_drive, sizeof(routes[i].punkt_drive));
        fin.getline(routes[i].time_start, sizeof(routes[i].time_start));
        fin.getline(routes[i].time_end, sizeof(routes[i].time_end));
    }
