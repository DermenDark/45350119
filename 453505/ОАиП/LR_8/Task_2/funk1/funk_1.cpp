...
vec_bus readd(const std::string& filename) {
    ...
    while (true) {
        ...
        if (strlen(route.nomer) ==0 ) {continue;}
        if(route.nomer=="*delite*" && route.typ_bus=="*delite*" && route.punkt_drive=="*delite*" && route.time_start=="*delite*" && route.time_end=="*delite*"){continue;}
